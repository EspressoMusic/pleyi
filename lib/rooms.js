...
function createRoom(code, teacherId, roomTitle, teacherToken) {
  ...
  return {
    ...
    showParticipantsToGuests: true,
    enableReactions: true,
    enableChat: true,
    enableGameSound: true,
    studentsCanPlay: false,
    students: new Map(),
    activeGame: null,
    gameState: {},
    scores: { teacher: 0 },
...
function roomSummary(room) {
  ...
  return {
    ...
    managerId: room.managerId || room.teacherId || null,
    ...
    learningItemCount: (room.learningItems || []).length,
  };
}

function isManager(room, socketId) {
  const managerId = room.managerId || room.teacherId;
  return !!managerId && managerId === socketId;
}
