...
const DEFAULT_SETTINGS = {
  ...
  lessonName: "שיעור עם משחקים",
  workingDays: [0, 1, 2, 3, 4],
  workingHours: { start: 10, end: 20 },
  breakBetweenLessonsMinutes: 15,
  slotIntervalMinutes: 65,
  bookingHorizonDays: 30,
  bitPhone: "050-0000000",
  whatsappPhone: "0586122187",
  bitPayName: "GameClass",
};
...
function generateSlotsForDate(dateStr, settings, bookings) {
  ...
  const lessonMins = settings.lessonDurationMinutes;
  const breakMins = settings.breakBetweenLessonsMinutes ?? 0;
  const stepMins = lessonMins + breakMins;
  const slots = [];
  const now = new Date();
  ...
  while (cursor < dayEnd) {
    const lessonEnd = new Date(cursor.getTime() + lessonMins * 60000);
    if (lessonEnd > dayEnd) break;
    if (cursor > now) {
    ...
    cursor = new Date(cursor.getTime() + stepMins * 60000);
  }
  return slots;
}
