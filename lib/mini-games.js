...
function handleSpotDiff(state, role, action, data, addScore, VOCAB) {
  if (action === "pick" && state.phase === "playing") {
    ...
    if (state.clicks[other] !== undefined) {
      state.roundWinner = state.clicks[other] === state.diffIndex ? other : "none";
      state.phase = "round-end";
    }
...
function handleRunner(state, role, action, data, addScore, VOCAB) {
  if (action === "jump" && state.phase === "playing") {
    ...
    const other = role === "teacher" ? "student" : "teacher";
    if (state.clicks[other] !== undefined) {
      state.roundWinner = state.clicks[other] === state.correct ? other : "none";
      state.phase = "round-end";
    }
    return { ok: true, correct: false };
  }
...
function handleCandy(state, role, action, data, addScore, VOCAB) {
  if (state.phase !== "playing" || state.turn !== role) return null;
  ...
  if (action === "select") {
    ...
    if (matchedSize > 0) {
      ...
      tile.word = tmpB.word;
      state.turn = role === "teacher" ? "student" : "teacher";
      state.lastMatch = 0;
      state.moves[role]++;
    }
...
function handleShop(state, role, action, data, addScore, VOCAB) {
